...
// TODO read ns3 api
        ...
        {
                static TypeId tid = TypeId ("ns3::pgpsr::DeferredRouteOutputTag").SetParent<Tag> (); //親のtidを設定し、<Tag>を取得するのはテンプレート
                return tid;
        }
...
#define FIRST_JITTER (Seconds (x->GetValue (0, PGPSR_MAXJITTER))) //最初のHelloは過去のものではなく,SetIpv4でのみ使用



NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// IANAではまだ定義されていない、PGPSR制御トラフィックのUDPポート
...
RoutingProtocol::NotifyInterfaceUp (uint32_t interface)
{
        ...
        //pgpsrは1つのインターフェイスに1つのIPアドレスしか持てません
